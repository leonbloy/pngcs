 namespace Ar.Com.Hjg.Pngcs {
	...
	internal class PngIDatChunkInputStream : Stream {
		...
		public PngIDatChunkInputStream(Stream iStream, int lenFirstChunk, int offset_0) {
			...
			// we know it's a IDAT
			System.Array.Copy((Array)(Ar.Com.Hjg.Pngcs.Chunks.ChunkHelper.IDAT),0,(Array)(idLastChunk),0,4);
		...
		private void EndChunkGoForNext() {
			...
			// Skips empty IDATS
			do {
				int crc = Ar.Com.Hjg.Pngcs.PngHelper.ReadInt4(inputStream); //
				offset += 4;
				...
				crcEngine.Reset();
				lenLastChunk = Ar.Com.Hjg.Pngcs.PngHelper.ReadInt4(inputStream);
				if (lenLastChunk < 0)
					throw new PngjInputException("invalid len for chunk: " + lenLastChunk);
				toReadThisChunk = lenLastChunk;
				Ar.Com.Hjg.Pngcs.PngHelper.ReadBytes(inputStream, idLastChunk, 0, 4);
				offset += 8;

				ended = ! PngCsUtils.arraysEqual4(idLastChunk,Ar.Com.Hjg.Pngcs.Chunks.ChunkHelper.IDAT);
				if (!ended) {
		...
		public void ForceChunkEnd() {
			if (!ended) {
				...
				Ar.Com.Hjg.Pngcs.PngHelper.ReadBytes(inputStream, dummy, 0, toReadThisChunk);
